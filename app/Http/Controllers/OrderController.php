...
class OrderController extends Controller
{
    ...
    public function checkout(Request $request, $fee)
    {
        ...
        $whatsappMessage = "*ORDER%20%20FROM%20CTTASTE*%20%0a*ORDER%20DETAILS*%0aOrder%20ID%20:%20" . $order->order_id . "%0a--�--%0a" . $packs . "*SUB%20TOTAL%20:%20₦" . $s_price . "*%0a*DELIVERY%20PRICE%20:%20₦" . $d_price . "*%0a*TOTAL%20PRICE%20:%20₦" . $f_price . "*%0a------*CUSTOMER%20DETAILS*------%0aName%20:%20" . $request->name . "%0aLocation%20:%20" . $request->location . "%0aAddress%20:%20" . $request->address . "%0aPhone%20number%20:%20" . $request->phone . "%20";

        if ($request->discount == 'true') {
            $whatsappMessage = "*ORDER%20%20FROM%20CTTASTE*%20%0a*FREE%20DRINK%20SPONSORED%20BY%20LOUDDA🍾💃🏽*%0a%0a*ORDER%20DETAILS*%0aOrder%20ID%20:%20" . $order->order_id . "%0a--�--%0a" . $packs . "*SUB%20TOTAL%20:%20₦" . $s_price . "*%0a*DELIVERY%20PRICE%20:%20₦" . $d_price . "*%0a*TOTAL%20PRICE%20:%20₦" . $f_price . "*%0a------*CUSTOMER%20DETAILS*------%0aName%20:%20" . $request->name . "%0aLocation%20:%20" . $request->location . "%0aAddress%20:%20" . $request->address . "%0aPhone%20number%20:%20" . $request->phone . "%20";
            DiscountedUser::create([
                'restaurant_id' => $cart->restaurant,
                'name' => $request->name,
                'phone' => $request->phone,
